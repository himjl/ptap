class TrialGeneratorClass {
    ...
    async get_trial(taskNumber, bagSamplingWeights) {
        ...
        var tP = this.trialBuffer[taskNumber][sampleBagName].shift(); // .shift() removes and returns the first element from the array
    ...
    async buffer_trial(taskNumber, sampleBagName) {
        ...
        NUM_TRIALS_BUFFERED = NUM_TRIALS_BUFFERED+1;
        if (NUM_TRIALS_BUFFERED <= TOTAL_TRIALS_TO_BUFFER){
            var pbar_string = ["Downloading trials...(", NUM_TRIALS_BUFFERED.toString(), "/", TOTAL_TRIALS_TO_BUFFER.toString(), ')'].join("");
        }
        else{
            var pbar_string = ""
        }
        wdm_large_red(pbar_string);
        console.log(NUM_TRIALS_BUFFERED)
    }
