class ImageBuffer {
    ...
    constructor(IO_object) {
        ...
        this.max_buffer_size = 410; // (for now, arbitrary) number of unique images to keep in buffer
    }
    ...
    async get_by_name(filename) {
        ...
        try {
            ...
            if (filename in this.cache_dict) {
                ...
                await this.download_images(filename);
                return this.cache_dict[filename]
            }
...
// Add specific image, or list of images, to cache before moving on.
    ...
    async download_images(imagenames) {
        ...
        try {
            ...
            if (typeof (imagenames) == "string") {
                ...
                if (!(filename in this.cache_dict)) {
                    var image = await this.IO_object.load_image(filename);
                ...
                var image_array = await this._downloadImageArray(requested_imagenames);
    ...
    async _downloadImageArray(imagepathlist) {
        try {
            ...
            if (imagepathlist.length > MAX_SIMULTANEOUS_REQUESTS) {
                ...
                for (var i = 0; i < Math.ceil(imagepathlist.length / MAX_SIMULTANEOUS_REQUESTS); i++) {
                    ...
                    for (var j = 0; j < partial_pathlist.length; j++) {
                        partial_image_requests.push(this.IO_object.load_image(partial_pathlist[j]))
                    }
                ...
                var image_requests = imagepathlist.map(this.IO_object.load_image);
